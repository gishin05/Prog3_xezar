...
if ($report_type === 'po_report') {
    ...
    $result = $conn->query(
        "SELECT po.order_id, po.supplier_name, po.order_quantity, po.received_quantity, po.status, 
                po.created_at, po.received_date, i.item_name, i.cost_price AS unit_cost 
    ...
    $result = $conn->query(
        "SELECT co.order_id, co.customer_name, co.quantity, co.status, 
                co.created_at, i.item_name, i.price AS sale_price, i.cost_price AS cost_price 
...
<body>
    ...
    <!-- Purchase Orders Report Table (matches order.php layout) -->
    <table>
        <thead>
            <tr>
                <th>Order ID</th>
        ...
        <tbody>
            ...
            if (count($all_purchase_orders) > 0):
                foreach ($all_purchase_orders as $po): 
                    $unit_cost = floatval($po['unit_cost'] ?? 0);
                    $total_val = $unit_cost * intval($po['order_quantity']);
            ?>
            <tr>
                ...
                <td>₱<?php echo number_format($total_val, 2); ?></td>
            ...
            ?>
            <tr>
                <td colspan="10" style="text-align: center; padding: 20px;">No purchase orders found.</td>
            </tr>
            <?php endif; ?>
        </tbody>
    </table>
    ...
    <!-- Sales Orders Report Table (matches order.php layout) -->
    <table>
        <thead>
            <tr>
                <th>Order ID</th>
        ...
        <tbody>
            ...
            if (count($all_customer_orders) > 0):
                foreach ($all_customer_orders as $co): 
                    $sale_price = floatval($co['sale_price'] ?? 0);
                    $sale_value = $sale_price * intval($co['quantity']);
            ...
            <tr>
                ...
                <td>₱<?php echo number_format($sale_value, 2); ?></td>
                <td>₱<?php echo number_format($product_cost, 2); ?></td>
                <td style="color: <?php echo $profit_color; ?>; font-weight: bold;">₱<?php echo number_format($profit, 2); ?></td>
            ...
            ?>
            <tr>
                <td colspan="10" style="text-align: center; padding: 20px;">No customer orders found.</td>
            </tr>
            <?php endif; ?>
            <?php if (count($all_customer_orders) > 0): ?>
            <tr class="total-row">
        ...
        <tbody>
            ...
            if (count($items) > 0):
                foreach ($items as $item): 
                    $price_display = isset($item['price']) ? floatval($item['price']) : 0;
                    $cost_for_value = isset($item['cost_price']) ? floatval($item['cost_price']) : $price_display;
                    $item_value = $cost_for_value * $item['quantity'];
