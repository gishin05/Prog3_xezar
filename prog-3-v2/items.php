...
// Determine sort order (available to both search and non-search queries)
$order_by = match($sort_by) {
    ...
    'aisle_asc' => 'isles ASC, item_name ASC',
    'aisle_desc' => 'isles DESC, item_name ASC',
    // Sort aisle by numeric value when possible (CAST to unsigned). Empty values fall back after numbers.
    'aisle_asc' => "(CASE WHEN TRIM(isles) = '' THEN 1 ELSE 0 END) ASC, CAST(isles AS UNSIGNED) ASC, isles ASC, item_name ASC",
    'aisle_desc' => "(CASE WHEN TRIM(isles) = '' THEN 1 ELSE 0 END) ASC, CAST(isles AS UNSIGNED) DESC, isles DESC, item_name ASC",
    // Sort shelf by leading letters (A-Z) then numeric part. Assumes format like 'A12' or 'B3'.
    'shelf_asc' => "(CASE WHEN TRIM(shelf_position) = '' THEN 1 ELSE 0 END) ASC, UPPER(LEFT(shelf_position,1)) ASC, CAST(SUBSTRING(shelf_position,2) AS UNSIGNED) ASC, shelf_position ASC, item_name ASC",
    'shelf_desc' => "(CASE WHEN TRIM(shelf_position) = '' THEN 1 ELSE 0 END) ASC, UPPER(LEFT(shelf_position,1)) DESC, CAST(SUBSTRING(shelf_position,2) AS UNSIGNED) DESC, shelf_position DESC, item_name ASC",
...
if (!empty($search)) {
    ...
    $sql = "SELECT * FROM items WHERE item_name LIKE ? OR isles LIKE ? OR shelf_position LIKE ? OR category LIKE ? ORDER BY " . $order_by;
            ...
            <div style="display: inline-block; position: relative;">
                <form method="GET" action="items.php" style="display: inline;">
                    ...
                    <select name="sort" onchange="this.form.submit()" class="form-input-modern" style="padding: 8px 12px; margin-left: 10px;">
                        ...
                        <option value="aisle_asc" <?php echo $sort_by === 'aisle_asc' ? 'selected' : ''; ?>>Sort by Aisle (Numeric ↑)</option>
                        <option value="aisle_desc" <?php echo $sort_by === 'aisle_desc' ? 'selected' : ''; ?>>Sort by Aisle (Numeric ↓)</option>
                        <option value="shelf_asc" <?php echo $sort_by === 'shelf_asc' ? 'selected' : ''; ?>>Sort by Shelf (Letter A-Z, then Number ↑)</option>
                        <option value="shelf_desc" <?php echo $sort_by === 'shelf_desc' ? 'selected' : ''; ?>>Sort by Shelf (Letter Z-A, then Number ↓)</option>
                    </select>
                </form>
            </div>
        </div>
                    ...
                    <?php else: ?>
                        <?php foreach ($all_items as $row): ?>
                            <tr>
                                ...
                                <?php
                                    $cost_display = isset($row['cost_price']) && $row['cost_price'] !== null && $row['cost_price'] !== ''
                                        ? number_format($row['cost_price'], 2)
                                        : number_format($row['price'], 2);
                                ?>
                                <td class="cost-cell">&#8369;<?php echo $cost_display; ?></td>
