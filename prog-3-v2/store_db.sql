...
CREATE TABLE `purchase_orders` (
  ...
  `status` enum('ordered','confirmed','processing','in_transit','delayed','received','partially_received','cancelled','returned') NOT NULL DEFAULT 'ordered',
  `notes` text,
...
CREATE TABLE `customer_orders` (
  ...
  `status` enum('placed','confirmed','picking','packed','shipped','delivered','cancelled','returned') NOT NULL DEFAULT 'placed',
  `notes` text,
  `created_by` int(11) NOT NULL,
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
...
-- Sample Items
INSERT INTO `items` (`item_id`, `item_name`, `category`, `price`, `cost_price`, `quantity`, `isles`, `shelf_position`) VALUES
(22, 'Apple', 'Fruits', 1.50, 1.50, 50, 'Aisle-8', 'Shelf-A-6'),
(23, 'Milk', 'Dairy', 1500.00, 1500.00, 5, 'Aisle-9', 'Shelf-D-7');
