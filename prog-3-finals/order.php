        ...
        if (!$isManager) {
            ...
            if ($supplier_name === '' || $order_quantity <= 0) {
                ...
                if ($product_mode === 'new') {
                    ...
                    $new_isles = trim($_POST['new_isles'] ?? '');
                    $new_shelf = trim($_POST['new_shelf_position'] ?? '');

                    if ($new_name === '' || $new_price === '' || !is_numeric($new_price) || $new_quantity < 0 || $new_isles === '' || $new_shelf === '') {
                        ...
                        try {
                            ...
                            $stmt->bind_param('sdiss', $new_name, $new_price, $new_quantity, $new_isles, $new_shelf);
                            ...
                            $stmt->bind_param('siiss', $supplier_name, $new_item_id, $order_quantity, $_SESSION['user_id'], $notes);
                    ...
                    if ($item_id <= 0) {
                        ...
                        $stmt = $conn->prepare("INSERT INTO purchase_orders (supplier_name, item_id, order_quantity, status, created_by, notes) VALUES (?, ?, ?, 'pending', ?, ?)");
                        $stmt->bind_param('siiss', $supplier_name, $item_id, $order_quantity, $_SESSION['user_id'], $notes);
                        if ($stmt->execute()) {
    ...
    elseif ($post_action === 'update_purchase_status') {
        if (!$isManager) {
            ...
            if ($order_id <= 0 || $new_status === '') {
                ...
                // Check if order is already received — if so, prevent any status change
                ...
                if ($check_row = $check_res->fetch_assoc()) {
                    if ($check_row['status'] === 'received') {
                        $error = 'Cannot modify status of a received order';
                    }
                }
                $check_stmt->close();
                    ...
                    if ($received_quantity <= 0) {
                        ...
                        try {
                            ...
                            if ($row = $res->fetch_assoc()) {
                                ...
                                $stmt->bind_param('siii', $new_status, $received_quantity, $_SESSION['user_id'], $order_id);
                                $stmt->execute();
                                $stmt->close();
        ...
        if ($customer_name === '' || $item_id <= 0 || $quantity <= 0) {
            ...
            if ($check_row = $check_result->fetch_assoc()) {
                ...
                if ($quantity > $available_quantity) {
                    ...
                    $stmt->bind_param('siiss', $customer_name, $item_id, $quantity, $_SESSION['user_id'], $notes);
                    if ($stmt->execute()) {
                    ...
                    if ($row = $res->fetch_assoc()) {
                        ...
                        if ($rrow = $r2->fetch_assoc()) {
                            ...
                            if ($stock < $qty) {
                                ...
                                $stmt->bind_param('sii', $new_status, $_SESSION['user_id'], $order_id);
                                $stmt->execute();
                                $stmt->close();
    ...
    elseif ($post_action === 'report_issue') {
        ...
        if ($order_id <= 0 || $issue_type === '' || $description === '') {
            ...
            if ($row = $res->fetch_assoc()) {
                ...
                if ($stmt) {
                    try {
                        $stmt->bind_param('iissii', $order_id, $item_id, $issue_type, $description, $quantity_affected, $_SESSION['user_id']);
                        if ($stmt->execute()) {
                            $stmt->close();
                        ...
                        <div class="form-group-modern">
                            ...
                            <div id="new_product_wrap" style="display:none; margin-top:12px;">
                                ...
                                <div class="form-group-modern">
                                    ...
                                    <input type="number" min="0" name="new_quantity" class="form-input-modern" placeholder="0">
                                </div>
                                <div class="form-group-modern">
                                    <label>Aisle</label>
                                    <input type="text" name="new_isles" class="form-input-modern" placeholder="e.g., Aisle 1">
                                </div>
                                <div class="form-group-modern">
                                    <label>Shelf Position</label>
                                    <input type="text" name="new_shelf_position" class="form-input-modern" placeholder="e.g., Shelf A-3">
                                </div>
                            </div>
    ...
    ?>
        <section class="form-section">
            <div class="form-container-modern">
                ...
                <?php if ($isManager && $po['status'] !== 'received'): ?>
                <h3>Update Status</h3>
                ...
                document.querySelector('select[name="new_status"]').addEventListener('change', function(e){
                    document.getElementById('received_group').style.display = this.value === 'received' ? 'block' : 'none';
                });
                </script>
                <?php elseif ($isManager && $po['status'] === 'received'): ?>
                <div class="alert alert-info">
                    <strong>Order Finalized:</strong> This purchase order has been received and cannot be modified.
                </div>
                <?php endif; ?>
